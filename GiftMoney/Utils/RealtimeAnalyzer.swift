...
class RealtimeAnalyzer {
    ...
    func peakPower(buffer: AVAudioPCMBuffer) -> Float {
        let channelsAmplitudes = analyse(with: buffer)
        var peakPower: Float = 0.0
        channelsAmplitudes.forEach { (amplitudes) in
            amplitudes.forEach({ (amplitude) in
                peakPower = amplitude > peakPower ? amplitude : peakPower
            })
        }
        return peakPower
    }
