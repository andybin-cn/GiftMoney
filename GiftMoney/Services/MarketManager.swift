...
class MarketManager: NSObject, SKPaymentTransactionObserver, SKProductsRequestDelegate {
    ...
    func paymentQueue(_ queue: SKPaymentQueue, updatedTransactions transactions: [SKPaymentTransaction]) {
        Log.info("updatedTransactions: start")
        for transaction in transactions {
            ...
            case .deferred:
                ...
                Log.info("updatedTransactions - deferred： \(productID)")
                self.payObserver?.onNext((productID, transaction.transactionState))
                break
            case .purchasing:
                //该交易正在由App Store处理。
                Log.info("updatedTransactions: purchasing： \(productID)")
                self.payObserver?.onNext((productID, transaction.transactionState))
                break
            case .failed:
                //交易失败
                Log.info("updatedTransactions: failed： \(productID)")
                self.payObserver?.onNext((productID, transaction.transactionState))
            ...
            case .restored, .purchased:
                Log.info("updatedTransactions: success： \(productID)")
                //购买成功
