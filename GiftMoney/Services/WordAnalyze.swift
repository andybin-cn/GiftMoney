...
class WordAnalyze {
    ...
    func analyzeSentence(tags: Array<JieBaTag>) -> AnalyzeResult {
        ...
        for tag in tags {
            if isBedeckWords(word: tag.word) {
            ...
            if tag.tag == "m" {
                result.value = tag.word
                continue
            }
            if isName(word: tag.word) > 0 {
                result.name = tag.word
                continue
            }
    ...
    func isName(word: String) -> Int {
        if names.findFirst(predicate: { (name) -> Bool in
    ...
    func isGiftUnit(word: String) -> Int {
        if giftUnits.findFirst(predicate: { (name) -> Bool in
            return word.contains(name)
        }) != nil {
            return 1
        }
    ...
    func isMoneyUnit(word: String) -> Int {
        if moneyUnits.findFirst(predicate: { (name) -> Bool in
            return word.contains(name)
        }) != nil {
            return 1
        }
