...
class SpeechButtonView: UIView {
    ...
    init() {
        ...
        exampleLabel.apply { (label) in
            ...
            attrStr.append(NSAttributedString(string: "请大声准确的对我说话，例句：\n", attributes: [NSAttributedString.Key.font : UIFont.appFont(ofSize: 13), NSAttributedString.Key.foregroundColor : UIColor.appSecondaryGray]))
...
//            self.animateView.layer.transform = CATransform3DMakeScale(scale, scale, 1)
    ...
    func stopRecognizer() {
        ...
        if !sentence.isEmptyString {
            let analyzeResult = WordAnalyze(sentence: sentence).analyzeSentence()
